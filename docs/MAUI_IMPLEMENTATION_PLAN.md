...
| 2026-08-09 | Shared educational disclaimer | A reusable compact footer now carries the website-aligned educational, hypothetical-results, and no-financial-advice language across defaults onboarding, the FIRE Quiz, the calculator catalog, every calculator detail, and Settings. iPad light-mode screenshots verify the footer at the true bottom of onboarding, the grouped calculator catalog, Standard FIRE, and Settings with non-zero bounds. The rendered 11-point text reports `#587068` on the light page surface with sufficient contrast. | iOS complete; Android pending |
