...
public static class DeferredCompensationWorkbook
{
    ...
    public static void Create(string filePath, DeferredCompensationDraft draft, DeferredCompensationResult result, DateTimeOffset generatedAt)
    {
        ...
        [new Row(Text("A1", "Retirement Cash Flow Inputs")), new Row(Text("A2", "Generated UTC"), Text("B2", generatedAt.UtcDateTime.ToString("O", CultureInfo.InvariantCulture))), new Row(Text("A4", "Input"), Text("B4", "Value")), new Row(Text("A5", "Current age"), Number("B5", draft.CurrentAge, DecimalStyleIndex)), new Row(Text("A6", "Semi-retirement age"), Number("B6", draft.SemiRetirementAge, DecimalStyleIndex)), new Row(Text("A7", "Plan through age"), Number("B7", draft.PlanThroughAge, DecimalStyleIndex)), new Row(Text("A8", "Annual expenses"), Number("B8", draft.AnnualExpenses, CurrencyStyleIndex)), new Row(Text("A9", "Inflation rate"), Number("B9", draft.InflationRate, PercentageStyleIndex))], 30, 22);
